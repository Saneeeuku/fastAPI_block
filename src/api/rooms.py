...
async def create_room(db: DBDep, hotel_id: int, data: RoomRequestAdd = Body(openapi_examples={
	"1": {
		"summary": "Одноместный",
		"value"  : {
			"title"      : "Одноместный",
...
async def get_free_rooms(
	...
	rooms = await db.rooms.get_by_time(hotel_id=hotel_id, date_from=date_from, date_to=date_to)
	return rooms
...
async def modify_room_partially(db: DBDep, hotel_id: int, room_id: int, data: RoomPATCH):
