...
async def test_post_register(email, password, nickname, status_code, ac, db):
    response = await ac.post(
        "/auth/registration", json={"email": email, "password": password, "nickname": nickname}
    )
...
async def test_login_and_me_and_delete(user_id, email, password, status_code, ac, db):
